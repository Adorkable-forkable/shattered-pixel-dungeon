...
public class WndSettings extends WndTabbed {
	...
	private DifficultyTab difficulty;
	...
	public WndSettings() {
		...
		difficulty = new DifficultyTab();
		difficulty.setSize(width, 0);
		height = Math.max(height, difficulty.height());
		add(difficulty);

		IconTab difficultyTab = new IconTab(Icons.get(Icons.GOLD)) {
			@Override
			protected void select(boolean value) {
				super.select(value);
				difficulty.visible = difficulty.active = value;
				if (value)
					last_index = 6;
			}
	...
	private static class DisplayTab extends Component {
		...
		protected void createChildren() {
			...
			if (DeviceCompat.isAndroid()) {
				...
				btnOrientation = new RedButton(finalLandscape ? Messages.get(this, "portrait")
	...
	private static class DifficultyTab extends Component {

		RenderedTextBlock title;
		ColorBlock sep1;
		CheckBox consumeAnkh;
		CheckBox hunger;
		...
		protected void createChildren() {
			...
			consumeAnkh = new CheckBox(Messages.get(this, "consume_ankh")) {
				@Override
				protected void onClick() {
					super.onClick();
					SPDSettings.consumeAnkh(!checked());
				}
			};
			consumeAnkh.checked(!SPDSettings.consumeAnkh());
			add(consumeAnkh);

			hunger = new CheckBox(Messages.get(this, "hunger")) {
				@Override
				protected void onClick() {
					super.onClick();
					SPDSettings.hunger(!checked());
				}
			};
			hunger.checked(!SPDSettings.hunger());
			add(hunger);
		}
		...
		protected void layout() {
			...
			if (width > 200) {
				consumeAnkh.setRect(0, sep1.y + 1 + GAP, width / 2 - 1, BTN_HEIGHT);
			} else {
				consumeAnkh.setRect(0, sep1.y + 1 + GAP, width, BTN_HEIGHT);
			}

			if (width > 200) {
				hunger.setRect(0, consumeAnkh.bottom() + 1 + GAP, width / 2 - 1, BTN_HEIGHT);
			} else {
				hunger.setRect(0, consumeAnkh.bottom() + 1 + GAP, width, BTN_HEIGHT);
			}
